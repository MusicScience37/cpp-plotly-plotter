...
/*!
 * \file
 * \brief Program to show color scales for positive numbers in plotly_plotter
 * library.
 */
#include <cmath>
#include <vector>
...
int main() {
    ...
    for (std::size_t row = 0; row < n_points_per_axis; ++row) {
        ...
        for (std::size_t col = 0; col < n_points_per_axis; ++col) {
            ...
            z[row][col] = std::exp(-coeff *
                (std::pow(x[col] - 0.7, 2) + std::pow(y[row] - 0.5, 2)));
        }
    }
    ...
    const auto color_scales = std::vector<
        ...
        {"warm_extended", warm_extended()}, {"autumn_full", autumn_full()},
        {"green_full", green_full()}, {"blue_full", blue_full()},
        {"blown", blown()}, {"inverted_gray", inverted_gray()}};
    ...
    for (std::size_t i = 0; i < color_scales.size(); ++i) {
        ...
        figure.layout().color_axis(i + 1).c_min(0.0);
        figure.layout().color_axis(i + 1).c_max(1.0);
    ...
    figure.title("Color Scales for Positive Numbers in cpp-plotly-plotter");

    // Save the figure to an HTML file.
    plotly_plotter::write_html(
        "color_scales_positive_color_scales.html", figure);
}
