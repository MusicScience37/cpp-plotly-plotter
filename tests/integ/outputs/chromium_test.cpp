...
TEST_CASE("chromium") {
    ...
    const char* chromium_path = std::getenv("CHROMIUM_PATH");
    if (chromium_path != nullptr) {
        converter.set_chrome_path(chromium_path);

        SECTION("create a PDF file") {
            const std::string html_file_path =
                "chromium_converter_pdf_test.html";
            const std::string pdf_file_path = "chromium_converter_pdf_test.pdf";
            ...
            plotly_plotter::details::write_html_impl(html_file_path.c_str(),
                ...
                plotly_plotter::details::html_template_type::pdf, width,
                height);
            ...
            if (converter.is_html_to_pdf_conversion_supported()) {
                CHECK_NOTHROW(
                    converter.convert_html_to_pdf(html_file_path.c_str(),
                        pdf_file_path.c_str(), width, height));
        ...
        SECTION("create a PNG file") {
            const std::string html_file_path =
                "chromium_converter_png_test.html";
            const std::string png_file_path = "chromium_converter_png_test.png";
            constexpr std::size_t width = 1600;
            constexpr std::size_t height = 900;

            plotly_plotter::details::write_html_impl(html_file_path.c_str(),
                figure.html_title().c_str(), figure.document(),
                plotly_plotter::details::html_template_type::png, width,
                height);
#if PLOTLY_PLOTTER_USE_UNIX_SUBPROCESS
            ...
            if (converter.is_html_to_png_conversion_supported()) {
                CHECK_NOTHROW(
                    converter.convert_html_to_png(html_file_path.c_str(),
                        png_file_path.c_str(), width, height));
